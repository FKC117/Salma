...
class EnhancedChatViewSet(viewsets.ViewSet):
    ...
    def update_session(self, request):
        ...
        try:
            ...
            # Update session through chat service
            result = self.chat_service.update_session(
    ...
    def sandbox_executions(self, request):
        """
        Get sandbox execution history for current user
        """
        try:
            # Get user
            try:
            ...
            executions = SandboxExecution.objects.filter(user=user).order_by('-created_at')[:50]
            ...
            logger.error(f"Sandbox executions error: {str(e)}", exc_info=True)
            return Response({
                'success': False,
                'error': 'Internal server error'
            }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        ...
        # Handle other common Python code patterns
        # Match patterns like "### Header\n\npython\nimport ..."
        def format_header_python_code_block(match):
        ...
        # Pattern to match header followed by Python code
        content = re.sub(r'###\s+(.*?)\s*\n\s*\n\s*python\s*\n(.*?)(?=\n\s*[A-Z#]|\n\s*$)', format_header_python_code_block, content, flags=re.DOTALL)
    ...
    def send_message(self, request):
        ...
        try:
            ...
            if result['success']:
                ...
                formatted_content = self._format_message_content(result['ai_response']['content'])
                ...
                html_response = render_to_string('analytics/partials/chat_message.html', {
                    ...
                    'timestamp': timezone.now().strftime('%H:%M'),
                    'message_metadata': True,
                    'token_count': result['tokens_used'].get('total_tokens', 0),
    ...
    def history(self, request):
        ...
        try:
            ...
            # Get chat history through chat service
    ...
    def execute_suggestion(self, request):
        ...
        try:
            # Validate request
            if 'suggestion_id' not in request.data:
                return Response({
                    'success': False,
            ...
            # Execute suggestion through suggestion service
            result = self.suggestion_service.execute_suggestion(
                suggestion_id=request.data['suggestion_id'],
                user=user,
                additional_params=request.data.get('parameters', {})
            )
            
            return Response(result, status=status.HTTP_200_OK if result['success'] else status.HTTP_400_BAD_REQUEST)
                
        except Exception as e:
            logger.error(f"Execute suggestion error: {str(e)}", exc_info=True)
            return Response({
                'success': False,
                'error': 'Internal server error'
            }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def context(self, request):
        ...
        try:
            ...
            # Get chat context through chat service
            ...
            logger.error(f"Chat context error: {str(e)}", exc_info=True)
            return Response({
                'success': False,
                'error': 'Internal server error'
            }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
